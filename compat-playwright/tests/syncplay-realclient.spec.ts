...
test.describe("syncplay real-client engagement", () => {
  ...
  test("second member joins an active group and catches up into playback", async ({
    browser,
    browserName,
  }) => {
    test.skip(
    ...
    // The real user scenario: A is already watching in a group; B joins. B must
    // receive the current PlayQueue + a catch-up Play (send_catch_up) and start
    // its own <video>, WITHOUT disturbing A (the "joiner skips everyone to the
    // next episode" bug). Then a pause from A must reach B.
    ...
    const bLog: string[] = [];
    pageB.on("console", (m) => bLog.push(`[${m.type()}] ${m.text()}`));
    pageB.on("pageerror", (e) => bLog.push(`[pageerror] ${e.message}`));

    // --- A: login, create group, start playing item 1 ---
    await connectToServer(pageA);
    await login(pageA, SEED_USER, SEED_PASS);
    await pageA.waitForURL(/#\/home/, { timeout: 25_000 });
    const serverId = await pageA.evaluate(
      () =>
        JSON.parse(window.localStorage.getItem("jellyfin_credentials")!)
          .Servers?.[0]?.Id ?? null,
    );
    await pageA.evaluate(() =>
      (window as any).ApiClient.createSyncPlayGroup({ GroupName: "pw-two" }),
    );
    await pageA.waitForTimeout(1500);
    await pageA.goto(`/#/details?id=1&serverId=${serverId}`);
    const playA = pageA.locator("button.btnPlay").first();
    await playA.waitFor({ timeout: 20_000 });
    await playA.click();
    ...
    // --- B: login, find A's group, join it ---
    await connectToServer(pageB);
    await login(pageB, SEED_USER, SEED_PASS);
    await pageB.waitForURL(/#\/home/, { timeout: 25_000 });
    const joined = await pageB.evaluate(async () => {
      const api = (window as any).ApiClient;
      const res = await api.getSyncPlayGroups();
      const groups = await res.json();
      if (!Array.isArray(groups) || groups.length === 0) {
        return { ok: false, reason: `no groups: ${JSON.stringify(groups)}` };
      }
      try {
        await api.joinSyncPlayGroup({ GroupId: groups[0].GroupId });
        return { ok: true, groupId: groups[0].GroupId };
      } catch (e: any) {
        return { ok: false, reason: String(e?.message ?? e) };
      }
    });
    expect(joined.ok, `B join failed: ${joined.reason}`).toBe(true);

    // B must start its own playback from the group's catch-up command.
    let bAdvanced = false;
    try {
    ...
    // A must NOT have been kicked to a different item by B joining.
    const aStillPlaying = await pageA.evaluate(() => {
      const v = document.querySelector("video") as HTMLVideoElement | null;
      return !!v && !v.ended && v.currentTime > 0;
    });
    ...
    expect(aStillPlaying, "A kept playing while B joined").toBe(true);
    expect(
      bAdvanced,
      `joiner B never started playback after catch-up.${bDiag}`,
    ).toBe(true);
  });
});
