...
- [X] T020a [P] Assert in `crates/pharos-transcode/src/scheduler.rs` that the `pharos_transcode_pin_total` outcome labels are distinct and stable. The assertion required a `PinOutcome` enum first: the labels were inline literals at their call sites, so a test comparing them would have compared constants written in the test against constants written beside them — tautology. Also counts the previously-silent no-device branch as `unresolved`, so the three outcomes are exhaustive and sum to the shared-init jobs placed. Verified RED with the two arms collapsed.
