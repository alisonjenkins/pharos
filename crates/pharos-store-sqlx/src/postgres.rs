...
use pharos_core::{
    ...
    ScanState, SecretString, SeriesInfo, SeriesMatchCandidate, SeriesMetadata, SeriesMetadataStore,
    Studio, StudioCount, StudioStore, SyncGroupStore, Tag, TagCount, TagStore, TokenStore,
...
impl pharos_core::MediaSegmentStore for PostgresStore {
    ...
    async fn set_segment_scan(&self, item_id: MediaId, schema_version: i64) -> DomainResult<()> {
        let id = i64::try_from(item_id)
            .map_err(|e| DomainError::Backend(format!("id overflow: {e}")))?;
        sqlx::query(
            "INSERT INTO media_segment_scans (item_id, schema_version) VALUES ($1, $2) \
    ...
    async fn segment_scan_version(&self, item_id: MediaId) -> DomainResult<Option<i64>> {
        let id = i64::try_from(item_id)
            .map_err(|e| DomainError::Backend(format!("id overflow: {e}")))?;
        let row = sqlx::query_as::<_, (i64,)>(
            "SELECT schema_version FROM media_segment_scans WHERE item_id = $1",
        )
        .bind(id)
        .fetch_optional(&self.pool)
        .await
    ...
    async fn snapshot_media_segments(&self, label: &str) -> DomainResult<u64> {
        let existing = sqlx::query_as::<_, (i64,)>(
            "SELECT COUNT(*) FROM media_segments_snapshot WHERE label = $1",
        )
        .bind(label)
        .fetch_one(&self.pool)
        .await
        ...
        let res = sqlx::query(
            "INSERT INTO media_segments_snapshot \
             ...
             SELECT $1, item_id, kind, start_ms, end_ms, detector, confidence, schema_version \
             FROM media_segments",
        )
        .bind(label)
        .execute(&self.pool)
...
impl UserDataStore for PostgresStore {
    ...
    async fn get_user_data(&self, user: UserId, item: MediaId) -> DomainResult<UserItemData> {
        ...
        let item_i64 = media_id_i64(item)?;
        let row: Option<PgUserDataRow> = sqlx::query_as(
        ...
        sqlx::query(
            "INSERT INTO user_data
             ...
             ON CONFLICT (user_id, item_id) DO UPDATE SET
               ...
               audio_stream_index =
                 COALESCE(EXCLUDED.audio_stream_index, user_data.audio_stream_index),
               subtitle_stream_index =
                 COALESCE(EXCLUDED.subtitle_stream_index, user_data.subtitle_stream_index)",
        )
        .bind(id_bytes)
        .bind(item_i64)
        .bind(played)
    ...
    async fn user_data_bulk(
        ...
        let rows: Vec<PgUserDataRowWithId> = sqlx::query_as(
        ...
        for (id, played, pc, pos, fav, lp, ai, si) in rows {
            by_id.insert(id, row_to_user_data((played, pc, pos, fav, lp, ai, si)));
        }
        Ok(ids_i64
            .iter()
...
impl SeriesMetadataStore for PostgresStore {
        ...
        let rows: Vec<(String, String, Option<i64>)> = sqlx::query_as(
            "SELECT s.series_key, s.series_name, s.series_year \
             ...
             WHERE (sm.match_source IS NULL OR sm.match_source IN ('search','none')) \
               AND (sm.metadata_refreshed_at IS NULL OR sm.metadata_refreshed_at < $1) \
             ORDER BY s.series_key ASC LIMIT $2",
        )
        .bind(ttl_cutoff)
        .bind(limit)
        .fetch_all(&self.pool)
    ...
    async fn series_metadata_by_keys(
        ...
        // Postgres `= ANY($1)` with a TEXT[] binding — no per-placeholder trick.
...
type PgUserDataRow = (i32, i32, i64, i32, i64, Option<i32>, Option<i32>);
/// The same row with its `item_id` in front, for the bulk read.
type PgUserDataRowWithId = (i64, i32, i32, i64, i32, i64, Option<i32>, Option<i32>);

fn row_to_user_data(row: PgUserDataRow) -> UserItemData {
