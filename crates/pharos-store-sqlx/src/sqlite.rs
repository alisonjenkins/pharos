...
impl MediaStore for SqliteStore {
    ...
    async fn sweep_unseen(&self, scan_id: i64, root_prefix: &str) -> DomainResult<Vec<MediaId>> {
        ...
        let (unseen, total): (i64, i64) = sqlx::query_as(
            "SELECT \
               COUNT(*) FILTER (WHERE last_seen_scan_id IS NULL OR last_seen_scan_id != ?), \
               COUNT(*) \
             FROM media_items WHERE path LIKE ? ESCAPE '\\'",
        )
        .bind(scan_id)
        .bind(&like)
        .fetch_one(&self.pool)
    ...
    async fn bump_art_version(&self, item_id: MediaId) -> DomainResult<()> {
        let id_i64 = i64::try_from(item_id)
            .map_err(|e| DomainError::Backend(format!("id overflow: {e}")))?;
        sqlx::query("UPDATE media_items SET art_version = art_version + 1 WHERE id = ?")
            .bind(id_i64)
            .execute(&self.pool)
            .await
            .map_err(|e| DomainError::Backend(e.to_string()))?;
    ...
    async fn clear_provider_artwork(&self, provider: &str) -> DomainResult<u64> {
        ...
        let affected = sqlx::query(
            "UPDATE media_items SET has_primary_art = 0, match_source = NULL, \
                match_external_id = NULL, metadata_refreshed_at = NULL \
             WHERE id IN (SELECT item_id FROM artwork WHERE source = ?)",
        )
        .bind(provider)
        .execute(&self.pool)
        .await
        .map_err(|e| DomainError::Backend(e.to_string()))?
        .rows_affected();
        sqlx::query("DELETE FROM artwork WHERE source = ?")
            .bind(provider)
            .execute(&self.pool)
            .await
            .map_err(|e| DomainError::Backend(e.to_string()))?;
...
impl pharos_core::MediaSegmentStore for SqliteStore {
    ...
    async fn episode_fingerprint_for(
        ...
        let row = sqlx::query_as::<_, (Vec<u8>,)>(
            "SELECT points FROM episode_fingerprints WHERE item_id = ? AND kind = ? \
             AND schema_version = ?",
        )
        .bind(id)
        .bind(kind.as_str())
