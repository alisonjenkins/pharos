...
//! audio (chromaprint: 11025 Hz / 4096-frame / ⅓-hop ≈ 0.124 s). Two episodes
...
/// Seconds of audio covered by one fingerprint point (chromaprint hop).
/// `4096 / 11025 / 3` — the plugin's `ChromaprintConstants.SampleDuration`.
pub const SAMPLE_DURATION_SECS: f64 = 4096.0 / 11025.0 / 3.0;
...
#[inline]
fn point_time(index: usize) -> f64 {
    index as f64 * SAMPLE_DURATION_SECS
}
...
fn find_contiguous(times: &[f64], max_skip: f64) -> Option<Span> {
    if times.is_empty() {
        return None;
    }
    ...
    Some(Span {
        start: best_start,
        end: best_end + SAMPLE_DURATION_SECS,
    })
}
...
fn matches_at_shift(
    ...
    for i in lo..lhs.len() {
        ...
        if (lhs[i] ^ rhs[j]).count_ones() <= cfg.max_bit_diff {
            lhs_times.push(point_time(i));
            rhs_times.push(point_time(j));
        }
    }
    (lhs_times, rhs_times)
}
...
pub fn compare(lhs: &[u32], rhs: &[u32], cfg: &AlignConfig) -> Option<MatchResult> {
    ...
    for shift in candidate_shifts(lhs, rhs, cfg) {
        ...
        let (Some(lhs_span), Some(rhs_span)) = (
            find_contiguous(&lhs_times, cfg.max_time_skip),
            find_contiguous(&rhs_times, cfg.max_time_skip),
        ) else {
            continue;
        };
        let (Some(lhs_span), Some(rhs_span)) =
...
mod tests {
    ...
    fn finds_shared_intro_at_different_offsets() {
        ...
        assert!(
            (m.rhs.start - 60.0 * SAMPLE_DURATION_SECS).abs() < 1.0,
            "rhs {}",
            m.rhs.start
        );
        // Both spans cover ~18.6 s.
        assert!(
            m.lhs.duration() > 15.0 && m.lhs.duration() < 25.0,
            "dur {}",
            m.lhs.duration()
        );
    }
    ...
    fn find_contiguous_breaks_on_large_gap() {
        ...
        let span = find_contiguous(&times, 3.5).unwrap();
