...
T30|.|BaseItemDto array-field enrichment — Artists, AlbumArtists, Genres, Tags, People, Studios, ProductionLocations default to empty arrays. Fixes the audio-detail Symbol.iterator throw caught by the crawl suite + general jellyfin-web compat.|I.jellyfin-api,V1,V7
...
T32|.|Jellyfin search endpoints — /Search/Hints (full-text against title + people + studios), /Items?searchTerm enrichment, /Search/Suggestions. Search page loads (T29) but no result wiring today.|I.jellyfin-api,V1,V7
...
T35|.|persistent server identity across restarts — server_id stored in store (or dedicated `system_identity` row) instead of regenerating UUIDv4 every process start. Today every restart forces clients to re-pair.|I.store,V1
T36|.|`pharos scan` CLI subcommand wires existing FsScanner + FfmpegProber into the cli::Cmd::Scan dispatch path (currently a no-op stub). Honors [media].roots from config; reports counts + failures.|I.cli,I.media-fs,V5
T37|.|CI workflow at `.github/workflows/ci.yml` running, via `nix develop --command`: cargo build, cargo nextest run --workspace, cargo clippy -D warnings, Playwright crawl (matrix linux+macos), `nix build .#pharos` + `nix build .#oci`. Caches /nix/store across runs.|V11,V17
...
T44|.|V8 redaction integration test — `tracing-test` capture across a full login + Sessions/Playing flow, assert no token bytes ever appear in any emitted event. Today V8 rests on SecretString's structural guard + reviewer discipline; this turns it into a CI check.|V8,V11
T45|.|cargo-audit + cargo-deny in CI with a checked-in deny.toml (license allowlist, advisory check, banned-crate list). Lock against future supply-chain regressions.|V17
